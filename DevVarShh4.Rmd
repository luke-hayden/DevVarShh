...
meascomp <- gather(mxstgp2, key="variable", value="cdpc", c("dev01", "dev05"))
...
ggsave(plot=p,height=10, width=10, file=paste("meascomp.pdf" ,sep="") )


ggsave(plot=p,height=10, width=10,device="png",  file=paste("meascomp.png" ,sep="") )
...
```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
p=ggplot(mxstgp2, aes(x=dev05, y=Weight, colour=Strain)) +geom_point()+ geom_smooth() + 
  scale_colour_manual(values=c("red3",  "cornflowerblue")) +
  xlab("Computed developmental age (cdpc)")+
  ylab("Embryo Weight (mg)") +
  theme_bw()+ 
  ggtitle(paste(jaw, "weight vs cpdc"))


ggsave(plot=p,height=10, width=14, file=paste(jaw,"wtvdev05.pdf" ,sep="") )
```
...
mmxstgmelted <- gather(mxstgmelted, key="agemeasure", value="oldness", c("Weight", "dev01", "dev05"))

mmxstgmelted$devlev <- case_when(
  mmxstgmelted$agemeasure== "dev01" ~ "Computed age: permissive model (cdpc)",
  mmxstgmelted$agemeasure == "Weight" ~ "Embryo weight (mg)",
  mmxstgmelted$agemeasure == "dev05" ~ "Computed age: realistic model (cpdc)")

(p <- ggplot(mmxstgmelted, aes(x=oldness,y=value,color=Strain,shape=Strain)) +
  geom_point(size=1.5) + 
  facet_grid(variable~devlev, scales= "free") +  
...
ggsave(plot=p,height=10, width=14,device="png",  file=paste(jaw,"charprog.png" ,sep="") )
...
ggsave(plot=p,height=10, width=14, file=paste(jaw,"charprog05.pdf" ,sep="") )


ggsave(plot=p,height=10, width=14,device="png",  file=paste(jaw,"charprog05.png" ,sep="") )
...
m5 <- gather(mxstg, key="agemeasure", value="oldness", c("Weight", "dev01", "dev05"))



m5$devlev <- case_when(
  m5$agemeasure== "dev01" ~ "Computed age: permissive model (cdpc)",
  m5$agemeasure == "Weight" ~ "Embryo weight (mg)",
  m5$agemeasure == "dev05" ~ "Computed age: realistic model (cpdc)")

(p <- ggplot(m5, aes(x=oldness,y=stateord,color=Strain)) +
  geom_point(size=1) + 
  facet_grid(Strain~devlev, scales="free") + 
...
ggsave(plot=p,height=7, width=24, device="png",file=paste(jaw,"allstate.png" ,sep="") )
...
stgmin <- select(mxstg, c("ID","Strain", "shh_R2_spot", "shh_M1_spot", "cap_transition", "shboob", "Weight", "dev01", "dev05"))
...
varaxe <- case_when(
  varname== "dev01" ~ "Computed age (cdpc)",
  varname == "Weight" ~ "Embryo weight (mg)",
  varname == "dev05" ~ "Computed age (cpdc)")

vardesc <- case_when(
  varname== "dev01" ~ "Computed age: permissive",
  varname == "Weight" ~ "Embryo weight",
  varname == "dev05" ~ "Computed age: realistic")
...
#  ylim(-0.5,2)+ 
  ...
  xlab("Strain") + 
  theme_bw()+
  ggtitle(label=paste("Overall variability:", vardesc), 
                subtitle=paste("wilcoxon test p-value < ", wilpval)))
...
png(paste(jaw, "timevar.png" ,sep=""),height=1500, width=600)
multiplot(plotlist=plotlist, cols=1)
dev.off()
...
png(paste(jaw, "allvar.png" ,sep=""),height=800, width=800)
multiplot(plotlist=plotlistbox, cols=1)
dev.off()

ggsave(plot=plotlist[[3]],height=4, width=7,device="png", file=paste(jaw,"timevardev05.png" ,sep="") )

ggsave(plot=plotlist[[3]],height=4, width=5.2,device="pdf", file=paste(jaw,"timevardev05.pdf" ,sep="") )
```
...
```{r, echo=FALSE, warning=FALSE, error=FALSE, message=FALSE}
cultexp <- data.frame(Strain= c("FVB", "FVB", "DUHi", "DUHi"), Condition= c("Incorporated", "Separate", "Incorporated", "Separate"),perc= c(.78,.22,.11,.89))  #num=c(7,2,1,8))
brks <- c(0, 0.25, 0.5, 0.75, 1)


(p=ggplot(cultexp, aes(Condition, perc, fill=Strain, alpha=Condition))+scale_y_continuous(labels = scales::percent)+
...
#  theme(axis.text.y = element_text(angle = 90, hjust = -0))+
  scale_fill_manual(values=c("red3", "cornflower blue"), guide=F)+
  scale_alpha_manual(values=c(1,0.5), guide=F)+
  ylab(""))#+
#  ggtitle("", subtitle= "p=0.015")
  

ggsave(plot=p,height=4, width=4,device="pdf", file=paste(jaw,"cultexp.pdf" ,sep="") )
```
